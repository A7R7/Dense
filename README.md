...
此功能是如何实现的？该脚本通过读取注册表（放心，它不修改注册表）确认当前虚拟桌面的序号，然后模拟若干次win+ctrl+←/→切换桌面。这意味着桌面切换动画会快速闪烁。如果您不喜欢这种动画效果，可以去下载github上的一个项目[virtual-desktop-enhancer](https://github.com/sdias/win-10-virtual-desktop-enhancer)，不过该项目只适用于win10（win11未测试过）。
